// Shared types for the ONYX agent and its websocket contract.
...
export type EventKind =
  | 'FILE_MODIFIED'
  | 'FILE_DELETED'
  | 'AST_COMPLEXITY_SPIKE'
  | 'AST_DEPENDENCY_CHANGE'
  | 'SYNTAX_FAILURE'
  | 'COMPILER_WARN'
  | 'COMPILER_FAILURE'
  | 'BUILD_CRASH'
...
// Cockpit-facing topology — derived from execution_snapshots + replay_events.
export interface TopologyNode {
  ...
  pulse: number; // 0..1 — visual activity intensity
...
export type WSMessage =
  ...
  | { type: 'blackout'; payload: { online: boolean; provider: 'mistral' | 'ollama' | 'cache'; since: number; reason: string } }
  | { type: 'demo'; payload: { phase: number; label: string; ts: number } }
